...
export class TransactionsService {
  private baseUrl =
    'https://r9vdzv10vd.execute-api.eu-central-1.amazonaws.com/dev';
  private transactionDetailsListSource = new BehaviorSubject<
    TransactionDetails[]
  >([]);
  ...
  sortTransactions(transactionsData: TransactionDetails[]): void {
    transactionsData.sort((a, b) =>
      a.dates.valueDate < b.dates.valueDate ? 1 : -1,
    );
  }

  private fetchAllTransactionsDetails(): Observable<TransactionDetails[]> {
    return this.http
      .get<TransactionResponse>(this.baseUrl + '/transactions')
      .pipe(
        catchError((_err) => {
          console.log(
            'Unable to fetch transactions data. Reusing local cache.',
          );
          return this.fetchLocalTransactionsData();
        }),
        map(({ data }) => {
          const transactionsData = data;
          this.sortTransactions(transactionsData);
